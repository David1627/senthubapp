...
def create_geotiff_download(data, filename, lat, lon, r_km, key):
    off = (r_km / 111.32) / 2
    # Ensure data is 2D for TIFF writing
...
if CLIENT_ID and CLIENT_SECRET:
    ...
    with tab1:
        if st.session_state.search_results:
            ...
            picks = st.multiselect("Pick two dates for Dual-View:", opts, default=opts[:min(len(opts), 2)])
            ...
            if len(st.session_state.image_cache) >= 2:
                ...
                st_folium(dm1, height=500, width=None, use_container_width=True, key="cat_dual")

    with tab2:
        if len(st.session_state.image_cache) >= 2:
            keys = list(st.session_state.image_cache.keys())
            c1, c2 = st.columns(2)
            d1 = c1.selectbox("Left (Baseline)", keys, index=0)
            d2 = c2.selectbox("Right (Crisis)", keys, index=1)
            ...
            st_folium(m_sync, height=600, width=None, use_container_width=True, key="comp_sync")
    ...
    with tab3:
        if len(st.session_state.image_cache) >= 2:
            ...
            w_dt = cf2.selectbox("Wet Crisis", keys, index=1, key="f_w")
            f_col = cf3.color_picker("Flood Color", "#00F6FF")
            
            col_s1, col_s2 = st.columns(2)
            sens = col_s1.slider("Sensitivity (dB)", -15.0, -2.0, -6.0)
            bg_opac = col_s2.slider("Radar Transparency", 0.0, 1.0, 0.4)
            ...
            # SAFE INDEXING FOR MASK
            h = f_col.lstrip('#'); rgb = [int(h[i:i+2], 16)/255 for i in (0, 2, 4)]
            m_rgb = np.zeros((*mask.shape, 4))
            m_rgb[mask == 1] = [*rgb, 0.9] 
            
            folium.raster_layers.ImageOverlay(get_img_url(m_rgb), bnds, name="Flood").add_to(mf)
            st_folium(mf, height=550, width=None, use_container_width=True, key="f_map")
            
            st.write("### 📥 Export")
            c_d1, c_d2 = st.columns(2)
            with c_d1: create_geotiff_download(mask, "flood.tif", st.session_state.lat, st.session_state.lon, r_km, "fd_r")
            with c_d2:
                ...
                st.download_button("📐 GeoJSON", json.dumps(gj), "flood.geojson", "application/json")
