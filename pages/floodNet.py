...
if CLIENT_ID and CLIENT_SECRET:
    ...
    with tab_ana:
        if len(st.session_state.image_cache_s1) >= 2:
            ...
            cmap_choice = c_lab3.selectbox("Color Ramp", ["viridis", "magma", "inferno", "plasma", "cividis", "Greys_r"])
            ...
            ax1.set_title(f"LEFT: {lab_before[:10]}")
            ax1.axis('off')
            
            im2 = ax2.imshow(data_right, cmap=cmap_choice, vmin=db_min, vmax=db_max)
            ax2.set_title(f"RIGHT: {lab_after[:10]}")
            ax2.axis('off')
            
            fig.colorbar(im2, ax=[ax1, ax2], label="dB", orientation='horizontal', pad=0.09)
            st.pyplot(fig)
        else:
    ...
    with tab_flood:
        if len(st.session_state.image_cache_s1) >= 2:
            ...
            # Save sensitivity to session state for the impact tab
    ...
    with tab_impact:
        if len(st.session_state.image_cache_s1) >= 2 and st.session_state.last_search_coords_s1:
            ...
            try:
                ...
                st.error(f"Error loading roads.geojson: {e}")
                roads_gdf = None

            if roads_gdf is not None:
                # Re-calculate flood mask based on settings in Tab 3
                b_db = 10 * np.log10(st.session_state.image_cache_s1[before][:,:,0] + 1e-10)
                a_db = 10 * np.log10(st.session_state.image_cache_s1[after][:,:,0] + 1e-10)
                f_mask = ((a_db - b_db) < st.session_state.flood_sens).astype(np.uint8)
                
                # Transform raster mask to GeoPandas
                lat, lon, r_km = st.session_state.last_search_coords_s1
                off = (r_km / 111.32) / 2
                ...
                if flood_polys:
                    ...
                    if not affected_roads.empty:
                        # Highlighted affected segments
                        folium.GeoJson(affected_roads, name="Flooded Segments", style_function=lambda x: {'color': 'red', 'weight': 5}).add_to(m_imp)
                        
                        # Add Points for affected areas
                        for _, row in affected_roads.iterrows():
                            point = row.geometry.centroid
                            folium.CircleMarker([point.y, point.x], radius=4, color="darkred", fill=True, popup="Affected Road Segment").add_to(m_imp)
                        
                        st.warning(f"🚨 Identified {len(affected_roads)} affected road segments.")
                    else:
                        st.success("✅ No road intersections detected with the current flood mask.")
                    
                    st_folium(m_imp, height=500, width=None, key="impact_map_viewer")
                else:
                    st.info("No significant flood detected to perform impact analysis.")
        else:
            st.info("💡 Please fetch data and configure the 'Flood Mapping' tab first.")
