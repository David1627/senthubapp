...
# We store these so the app doesn't crash on rerun
...
if CLIENT_ID and CLIENT_SECRET:
    ...
    if btn_run:
        # Update Lat/Lon in Session State
        if search_mode == "City":
            try:
                loc = Nominatim(user_agent="flood_pro").geocode(city_q)
                if loc: 
                    st.session_state.lat, st.session_state.lon = loc.latitude, loc.longitude
            except: st.error("Geocoding failed.")
        
        st.session_state.last_search_coords_s1 = (st.session_state.lat, st.session_state.lon, radius)
        off = (radius / 111.32) / 2
            ...
            if st.session_state.image_cache_s1:
                ...
                for i, (dk, img_data) in enumerate(st.session_state.image_cache_s1.items()):
                    with cols[i]:
                        ...
                        gain = st.slider(f"Gain {dk[:10]}", 0.5, 10.0, 3.0, key=f"g_{dk}")
                        mode = st.radio(f"Pol {dk[:10]}", ["VV", "VH"], key=f"m_{dk}", horizontal=True)
                        ...
                        st_folium(m, height=400, key=f"map_dash_{dk}")
    ...
    with tab2: # ADVANCED LAB
        if st.session_state.image_cache_s1:
            ...
            date_r = c2.selectbox("Right Image", list(st.session_state.image_cache_s1.keys()), index=min(1, len(st.session_state.image_cache_s1)-1))
    ...
    with tab3: # FLOOD IMPACT
        if len(st.session_state.image_cache_s1) >= 2:
            ...
            b_key = st.selectbox("Baseline (Dry)", list(st.session_state.image_cache_s1.keys()), 0, key="impact_b")
            a_key = st.selectbox("Crisis (Wet)", list(st.session_state.image_cache_s1.keys()), 1, key="impact_a")
            ...
            st_folium(m_final, height=600, width=1200, key="final_map_render")
            
            c1, c2 = st.columns(2)
            with c1: export_geotiff(f_mask, "flood_mask.tif", *st.session_state.last_search_coords_s1, "dl_f_final")
            with c2: 
                if impact_b is not None:
                    st.download_button("📐 Building GeoJSON", impact_b.to_json(), "buildings.geojson")
else:
