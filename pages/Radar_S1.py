...
def get_image_url(np_img):
    """Safely converts a numpy array to a base64 encoded PNG string."""
    try:
        if np_img is None:
            return ""
        # Rescale 0.0-1.0 to 0-255
...
if CLIENT_ID and CLIENT_SECRET:
    ...
    if btn_search:
        ...
        if search_mode == "Search City":
            try:
                ...
                else: st.error("City not found. Using default coordinates.")
            except: st.error("Geocoding service busy. Using manual input.")
    ...
    with tab_dash:
        if st.session_state.search_results_s1:
            ...
            if st.button("🖼️ RENDER RADAR", use_container_width=True):
                ...
                with st.spinner("Downloading imagery..."):
                    for d_str in sel_dates:
                        idx = int(d_str.split(":")[0])
                        actual_date = res[idx]['properties']['datetime']
                    ...
                    if actual_date in st.session_state.image_cache_s1:
                        with cols[i % 2]:
                            ...
                            if img_url:
                                ...
                                st_folium(m, height=350, width=None, key=f"map_{i}_{actual_date}")

    with tab_ana:
        if st.session_state.image_cache_s1:
            ana_date = st.selectbox("Select Date for Pixel Analysis", list(st.session_state.image_cache_s1.keys()))
            data_raw = st.session_state.image_cache_s1[ana_date]
            db_val = 10 * np.log10(data_raw[:,:,0] + 1e-10)
            
            fig, ax = plt.subplots(figsize=(8, 4))
            im = ax.imshow(db_val, cmap='magma', vmin=-30, vmax=0)
            plt.colorbar(im, label="Backscatter Intensity (dB)")
            ax.set_title(f"Radar Intensity Profile: {ana_date[:10]}")
            st.pyplot(fig)
    ...
    with tab_flood:
        if len(st.session_state.image_cache_s1) >= 2:
            ...
            with c1: before = st.selectbox("Baseline (Dry)", d_list, index=0)
            with c2: after = st.selectbox("Crisis (Wet)", d_list, index=1)
            with c3: f_color = st.color_picker("Flood Highlight Color", "#00FFFF")
            ...
            sensitivity = st.slider("Flood Threshold (Lower = more water)", -15.0, -2.0, -6.0)
            flood_mask = ((a_db - b_db) < sensitivity).astype(float)
            
            if st.checkbox("Apply Water Mask (Remove permanent water)"):
                flood_mask[b_db < -16] = 0
            
            col_m, col_exp = st.columns([3, 1])
            with col_exp:
                st.write("### 📂 Export Data")
                create_geotiff_download(flood_mask, "flood_extent.tif", *st.session_state.last_search_coords_s1, key="dl_t_final")
            ...
            with col_m:
                ...
                # Background Radar
                bg_radar = np.dstack([np.clip(st.session_state.image_cache_s1[after][:,:,0]*brightness, 0, 1)]*3)
                folium.raster_layers.ImageOverlay(image=get_image_url(bg_radar), bounds=st.session_state.current_bounds_s1, opacity=0.4).add_to(m_f)
                ...
                f_url = get_image_url(mask_rgb)
                if f_url:
                    folium.raster_layers.ImageOverlay(image=f_url, bounds=st.session_state.current_bounds_s1).add_to(m_f)
                    st_folium(m_f, height=500, width=None, key="final_flood_map")
        else:
            st.info("💡 Render at least 2 images in the Dashboard tab first.")
else:
    st.info("👋 Enter your Sentinel Hub credentials in the sidebar and click Search.")
