...
def create_geojson_download(mask, lat, lon, radius_km):
    ...
    # Generate shapes (vectorize) from the binary flood mask
...
if st.session_state.last_search_coords_s1:
                ...
                for i, d_str in enumerate(sel_dates):
                    with cols[i % 2]:
                        ...
                        if actual_date in st.session_state.image_cache_s1:
                            ...
                            pol = st.radio(f"View {i+1} Mode", ["VV", "VH", "False Color"], horizontal=True, key=f"p_{i}")
    ...
    with tab_flood:
        if len(st.session_state.image_cache_s1) >= 2:
            ...
            before = c1.selectbox("Baseline (Dry Date)", d_list, index=0)
            after = c2.selectbox("Crisis (Wet Date)", d_list, index=min(1, len(d_list)-1))
            f_color = c3.color_picker("Flood Overlay Color", "#00FFFF")
            ...
            sensitivity = st.slider("Sensitivity (Lower = more strict)", -15.0, -2.0, -6.0)
            flood_mask = (diff < sensitivity).astype(float)
            
            if st.checkbox("Hide Permanent Water (Oceans/Rivers)"):
                flood_mask[b_db < -16] = 0
            
            col_m, col_d = st.columns([3, 1])
            with col_d:
                st.write("### 📥 Export Center")
                create_geotiff_download(flood_mask, "flood_raster.tif", *st.session_state.last_search_coords_s1, key="dl_t")
                create_geojson_download(flood_mask, *st.session_state.last_search_coords_s1)
            
            with col_m:
                m_f = folium.Map(location=[lat, lon], zoom_start=12)
                # Background Radar
                ...
                st_folium(m_f, height=500, width=800, key="f_map_final")
        else:
            st.info("Please load at least 2 images in the Dashboard to map floods.")
else:
    st.info("Enter your credentials and search for a city to begin.")
